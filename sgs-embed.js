...
(function (global) {
  ...
  var STYLE = [
    ...
    '.sgs-search { background: color-mix(in srgb, var(--sgs-surface) 100%, transparent); border: 1px solid rgba(255,255,255,.12); color: var(--sgs-text); padding: 9px 14px; border-radius: 999px; font-size: 13px; font-family: inherit; outline: none; min-width: 180px; transition: border-color .2s, box-shadow .2s; }',
    '.sgs-search:focus { border-color: var(--sgs-accent); box-shadow: 0 0 0 3px color-mix(in srgb, var(--sgs-accent) 30%, transparent); }',
    ...
    '.sgs-card { position:relative; background: var(--sgs-surface); border-radius: var(--sgs-radius); overflow:hidden; border: 1px solid rgba(255,255,255,.08); cursor:pointer; transform: translateY(0); transition: transform .35s cubic-bezier(.2,.8,.2,1), border-color .3s, box-shadow .35s; opacity:0; animation: sgs-in .5s ease forwards; }',
    '.sgs-card:nth-child(1){animation-delay:.02s} .sgs-card:nth-child(2){animation-delay:.06s} .sgs-card:nth-child(3){animation-delay:.1s} .sgs-card:nth-child(4){animation-delay:.14s} .sgs-card:nth-child(5){animation-delay:.18s} .sgs-card:nth-child(6){animation-delay:.22s}',
    '@keyframes sgs-in { from { opacity:0; transform: translateY(10px); } to { opacity:1; transform: translateY(0); } }',
    '.sgs-card:hover { transform: translateY(-6px); border-color: color-mix(in srgb, var(--sgs-accent) 55%, rgba(255,255,255,.08)); box-shadow: 0 18px 40px -18px rgba(0,0,0,.6), 0 0 0 1px color-mix(in srgb, var(--sgs-accent) 30%, transparent); }',
    '.sgs-spot { position:absolute; inset:0; pointer-events:none; opacity:0; transition: opacity .3s; background: radial-gradient(circle 180px at var(--mx,50%) var(--my,50%), color-mix(in srgb, var(--sgs-accent) 35%, transparent), transparent 70%); mix-blend-mode: screen; }',
    ...
    '.sgs-name { font-size: 14.5px; font-weight: 700; line-height:1.3; margin:0; }',
    ...
    '.sgs-more { background:transparent; border: 1px solid color-mix(in srgb, var(--sgs-accent) 60%, rgba(255,255,255,.2)); color: var(--sgs-text); padding: 10px 22px; border-radius: 999px; font-family: inherit; font-size: 13px; font-weight:600; cursor:pointer; transition: background .25s, transform .2s; }',
    '.sgs-more:hover { background: color-mix(in srgb, var(--sgs-accent) 18%, transparent); transform: translateY(-1px); }',
    '.sgs-badge { position:absolute; top:10px; left:10px; background: color-mix(in srgb, var(--sgs-accent) 88%, black 0%); color:#12151A; font-size:10.5px; font-weight:800; text-transform:uppercase; letter-spacing:.06em; padding: 4px 9px; border-radius: 999px; }',
  ...
  function createGallery(container, config) {
    ...
    var wrap = el('div', 'sgs');
    wrap.style.setProperty('--sgs-accent', theme.accent);
    wrap.style.setProperty('--sgs-bg', theme.background);
    wrap.style.setProperty('--sgs-surface', theme.surface);
    wrap.style.setProperty('--sgs-text', theme.text);
    wrap.style.setProperty('--sgs-muted', theme.muted);
    wrap.style.setProperty('--sgs-radius', theme.radius + 'px');
    wrap.style.setProperty('--sgs-gap', theme.gap + 'px');
    wrap.style.setProperty('--sgs-columns-min', theme.cardMinWidth + 'px');
    wrap.style.setProperty('--sgs-font', theme.font);
    ...
    function paint() {
      ...
      visible.forEach(function (it, i) {
        ...
        var name = el('p', 'sgs-name'); name.textContent = it.name;
        ...
        card.addEventListener('click', function () {
          openLightbox(list.map(function (l) { return { image: l.image, name: l.name, priceLabel: l.priceLabel || formatPrice(l.price, cfg.currency, cfg.locale) }; }), i, theme);
        });
